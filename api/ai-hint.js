...
- Default to ENGLISH, but adapt to any language the student uses or requests
...
The homogeneous equation: y'' + y = 0 with r² + 1 = 0

Complex roots: r = ±i

The homogeneous solution: y_h = C₁cos(x) + C₂sin(x)

For the particular solution using Variation of Parameters: y₁ = cos(x), y₂ = sin(x)

The Wronskian: W(y₁, y₂) = cos²(x) + sin²(x) = 1

Computing the integrals: ∫sin(x)tan(x)dx = ∫sin(x)·(sin(x)/cos(x))dx = ∫(sin²(x)/cos(x))dx

After evaluating the integrals we obtain: y_p = −cos(x)ln|1+sin(x)/cos(x)|

The general solution: y = C₁cos(x) + C₂sin(x) − cos(x)ln|1+sin(x)/cos(x)|

// MODE 2: SUBMISSION EXERCISE
// Uncomment this section for submission exercises
/*
"You have used up your attempts for this exercise today. You can check the two hint tabs, try again tomorrow, or wait for the solutions to be published."
*/
